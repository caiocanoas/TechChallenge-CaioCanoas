...
const token = 'Insira aqui o seu token do Telegram Bot'
...
bot.on('message', (msg) => {
    ...
    if(msg.text.indexOf(buscar) === 0) {
        bot.sendMessage(msg.chat.id, "Que legal! É só digitar o produto que deseja pesquisar e eu vou te mandar o link")
        .then(bot.once('message', (msg) => {
            let produto = msg.text.toLowerCase().replace(/ /g, "%20")
            bot.sendMessage(msg.chat.id, `Certo! Dê uma olhada aqui: https://busca.carrefour.com.br/busca?q=${produto}`,
            { "reply_markup": opcoes })
        }))
    }
    ...
    if(msg.text.indexOf(ofertas) === 0) {
        bot.sendMessage(msg.chat.id, "Ótimo, " + msg.from.first_name +
    ...
    if(msg.text.indexOf(pedido) === 0) {
        bot.sendMessage(msg.chat.id, "É muito fácil acompanhar os seus pedidos, " + 
        msg.from.first_name +
    ...
    if(msg.text.indexOf(redes) === 0) {
        bot.sendMessage(msg.chat.id, "Beleza!\nEm qual rede social você deseja encontrar o Carrefour?\nEscolha entre as opções abaixo", 
        { "reply_markup":  sociais })
        .then(
            bot.once('message', (msg) => {
                var fb = "Facebook"
                if(msg.text.indexOf(fb) === 0) {
                    bot.sendMessage(msg.chat.id, "Você encontra o Carrefour no Facebook aqui: https://www.facebook.com/CarrefourBR/", {
                        "reply_markup": opcoes })
                }

                var tw = "Twitter"
                if(msg.text.indexOf(tw) === 0) {
                    bot.sendMessage(msg.chat.id, "Você encontra o Carrefour no Twitter aqui: https://twitter.com/carrefourbrasil", {
                        "reply_markup": opcoes })
                }

                var yt = "YouTube"
                if(msg.text.indexOf(yt) === 0) {
                    bot.sendMessage(msg.chat.id, "Você encontra o Carrefour no YouTube aqui: https://www.youtube.com/user/carrefourbrasil", {
                        "reply_markup": opcoes })
                }

                var ig = "Instagram"
                if(msg.text.indexOf(ig) === 0) {
                    bot.sendMessage(msg.chat.id, "Você encontra o Carrefour no Instagram aqui: https://www.instagram.com/carrefourbrasil/", {
                        "reply_markup": opcoes })
                }
            })
        )
